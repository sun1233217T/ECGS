...
class GaussianModel:
    ...
    def __init__(self, sh_degree, optimizer_type="default",cat_low_app_opc = False,scale_dim = False):
        ...
        if self.scale_quant:
            ...
            self.scale_fsq = FSQDecoder(input_dim=scale_dim, inner_dim=32).cuda()
            self.scale_quantizer = FSQQuantizer().cuda()
            self.scale_inverse_fsq = InverseDecoder(output_dim=scale_dim, inner_dim=32).cuda()
    ...
    def training_setup(self, training_args):
        ...
        if self.scale_quant:
            l.append({'params': self.scale_fsq.parameters(), 'lr': training_args.scaling_lr, "name": "scale_fsq"})
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii = None, new_denom = None, new_grad_accum = None):
        d = {"xyz": new_xyz,
    ...
    def get_my_scalse_loss(self, lambda_scale = 0.01, epsilon = 1e-8, needle_shift = 0.5, plane_shift = 2, lambda_n = 0.9):
        ...
        loss = lambda_n * loss_n + (1 - lambda_n) * loss_p
