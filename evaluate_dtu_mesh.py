...
def cull_mesh(cameras, mesh):
    ...
    print(2)
    mesh_clean_list = [components[sel] for sel in np.where(areas>1.)[0]]
    if len(mesh_clean_list) == 0:
        mesh_clean_list = [components[areas.argmax()]]
    '''
    mesh_clean_list = [mesh]

    return mesh_clean_list


def evaluate_mesh(dataset : ModelParams, DTU_PATH : str, is_sugar):
    ...
    for iteration in iterations:
        ...
        filename = "tsdf.ply"
        if is_sugar:
            filename = f"{iteration}_sugarmesh_3Dgs7000_sdfestim02_sdfnorm02_level03_decim1000000.ply"
        
        # load mesh
        if is_sugar:
            mesh_file = os.path.join(dataset.model_path, filename)
        else:
            mesh_file = os.path.join(dataset.model_path, "test/{}".format(iteration), mesh_dir, filename)
        ...
        mesh_list = cull_mesh(train_cameras, mesh)

        for idx, mesh in enumerate(mesh_list):
            if is_sugar:
                culled_mesh_file = os.path.join(dataset.model_path, filename.replace(".ply", f"_culled_{idx}.ply"))
            else:
                culled_mesh_file = os.path.join(dataset.model_path, "test/{}".format(iteration), mesh_dir, filename.replace(".ply", f"_culled_{idx}.ply"))
            ...
            if is_sugar:
                aligned_mesh_file = os.path.join(dataset.model_path, filename.replace(".ply", f"_aligned_{idx}.ply"))
            else:
                aligned_mesh_file = os.path.join(dataset.model_path, "test/{}".format(iteration), mesh_dir, filename.replace(".ply", f"_aligned_{idx}.ply"))
            mesh.export(aligned_mesh_file)
                
            # evaluate
            if is_sugar:
                out_dir = os.path.join(dataset.model_path, "sugar_results", mesh_dir, str(iteration))
                makedirs(out_dir, exist_ok=True)
            else:
                out_dir = os.path.join(dataset.model_path, "test/{}".format(iteration), mesh_dir)
            #scan = dataset.model_path.split("/")[-1][4:]
            pattern = r"scan(\d+)"
            scan = re.search(pattern, dataset.model_path).group(1)
            
            cmd = f"python dtu_eval/eval.py --data {aligned_mesh_file} --scan {scan} --mode mesh --dataset_dir {DTU_PATH} --vis_out_dir {out_dir} --ind {idx}"
            print(cmd)
            os.system(cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--DTU', type=str,  default='data/DTU/Offical_DTU_Dataset', help='path to the GT DTU point clouds')
    ...
    evaluate_mesh(model.extract(args), args.DTU, args.sugar)
