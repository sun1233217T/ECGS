...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, separate_sh = False, override_color = None, use_trained_exp=False, return_app_opacity=False, return_restruct_depth = False):
    if return_app_opacity:
        return render_analyse(viewpoint_camera, pc, pipe, bg_color, scaling_modifier, separate_sh, override_color, use_trained_exp)
    """
    Render the scene. 
    ...
    out["render_opacity"] = render_opacity
    
    if return_restruct_depth:
        with torch.no_grad():
